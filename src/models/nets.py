...
class UNet(nn.Module):
    ...
    def __call__(self, x, cond, t):
        ...
        x2 = ResBlock(self.base_channels, self.base_channels * 2, self.base_channels)(
            x2, emb
        )
        ...
        h = jax.image.resize(
            h,
            shape=(h.shape[0], x2.shape[1], x2.shape[2], h.shape[3]),
            method="nearest",
        )
        h = jnp.concatenate([h, x2], axis=-1)
        h = ResBlock(self.base_channels * 4, self.base_channels, self.base_channels)(
            h, emb
        )

        h = jax.image.resize(
            h,
            shape=(h.shape[0], x1.shape[1], x1.shape[2], h.shape[3]),
            method="nearest",
        )
        h = jnp.concatenate([h, x1], axis=-1)
        h = ResBlock(self.base_channels * 2, self.base_channels, self.base_channels)(
            h, emb
        )
